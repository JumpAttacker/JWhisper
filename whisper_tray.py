...
# -------------------- LOGGING SETUP --------------------
def setup_logging():
    """Setup rotating log file for whisper activities"""
    ...
    log_file = os.path.join(os.path.dirname(__file__), 'whisper.log')
...
status_file = os.path.join(os.environ.get('TEMP', ''), 'whisper_running.lock')
...
if os.path.exists(status_file):
    try:
        ...
        if old_pid in [p.pid for p in psutil.process_iter() if p.name() == 'pythonw.exe']:
            ...
            print("ERROR: Whisper is already running in system tray!")
            sys.exit(1)
    except:
...
def on_show_status(icon, item):
    ...
    def show_status_window():
        ...
        status_text = f"""Whisper Voice-to-Text Status
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ Service: RUNNING
📍 Process ID: {pid}
⏱️ Uptime: {uptime_str}
...
def on_view_logs(icon, item):
    ...
    try:
        if os.path.exists(log_file):
            ...
            show_notification("Whisper Hotkey", "No log file found yet. Start using the application to generate logs.", quiet=True)
    except Exception as e:
...
def on_restart_service(icon, item):
    ...
    show_notification("Whisper Hotkey", "Restarting service...", quiet=True)
...
def toggle_autostart(icon, item):
    ...
    try:
        ...
        try:
            ...
            show_notification("Whisper Hotkey", "Removed from autostart", quiet=True)
        except FileNotFoundError:
            # Add to autostart
            winreg.SetValueEx(key, app_name, 0, winreg.REG_SZ, app_path)
            print("✓ Added to autostart")
            show_notification("Whisper Hotkey", "Added to autostart", quiet=True)
...
def setup_tray():
    ...
    tray_icon = pystray.Icon("WhisperHotkey", image, "Whisper Voice-to-Text\nF9 to record\nRight-click for options", menu)
...
def load_model():
    ...
    try:
        ...
        show_notification("Whisper Hotkey", "Model loaded successfully. Press F9 to record.", quiet=True)
    except Exception as e:
        logger.error(f"Failed to load model: {e}")
        show_notification("Error", f"Failed to load Whisper model: {e}", quiet=True)
        raise
...
def main():
    ...
    try:
        ...
        show_notification("Critical Error", "Failed to load Whisper model. Check logs for details.", quiet=True)
        return
